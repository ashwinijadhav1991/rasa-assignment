    ...
    $.ajax({
        ...
        success: function(data, textStatus) {
            ...
            if (window.previous_user_msg == "Hi") {
                ...
                popmsg = "Hey! I am Elena. Your HR Digital Assitant. Please click here so that I can assist you."
        ...
        error: function(errorMessage) {
            ...
            if (window.previous_user_msg == "Hi") {
                console.log("Inside if statement for popup msg");
                popmsg = "Hey, I am Elena. Your HR Digital Assitant.<br> Please click here so that I can assist you."
    ...
    setTimeout(function() {
        ...
        for (i = 0; i < suggLength; i++) {
            ...
            if (suggestions[i].notifications) {
                ...
                $('<div class="menuChips"><div class = "scrollmenu"><div class = "btn-group_back"><button onclick = send("' + suggestions[i].title + '")><span class="material-icons" id= "home">home</span></button></div></div></div>').appendTo('.menu');
...
function addForm() {
    ...
    for (i = 0; i < data1[0].fields.length; i++) {
        if (data1[0].fields[i].type == 'tel') {
            form_created += '<label id= "chat-bot-lb1" class="chat-bot-compulsary">' + data1[0].fields[i].field + '</label><input type="' + data1[0].fields[i].type + '" class= "infield" id="chat-bot-' + data1[0].fields[i].field + '" name="' + data1[0].fields[i].name + '" placeholder="' + data1[0].fields[i].placeholder + '" pattern="' + data1[0].fields[i].pattern + '" autocomplete = "off" required /><br>';
        } else
        if (data1[0].fields[i].type == 'dropdown') {
            for (j = 0; j < data1[0].fields[i].placeholder.length; j++) {
                dropdown += '<option value=' + data1[0].fields[i].placeholder[j].value +
                    '>' + data1[0].fields[i].placeholder[j].option + '</option>';
            }
